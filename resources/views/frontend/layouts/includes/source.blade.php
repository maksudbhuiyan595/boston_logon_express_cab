...
<style>
    ...
    .reservation-card {
        ...
        border-top: 6px solid var(--brand-blue); /* টপ বর্ডার একটু মোটা */
        width: 100%;
        z-index: 100;
        min-height: 530px;
        height: auto;
...
<style>
    ...
    .reservation-card {
        background: #0f1412; /* ইউনিক ডিপ এমারেল্ড ব্ল্যাক */
        border-radius: 16px;
        box-shadow: 0 20px 40px rgba(0, 0, 0, 0.4);
        border: 1px solid rgba(212, 175, 55, 0.2);
        border-top: 6px solid #d4af37; /* গোল্ডেন টপ বর্ডার */
        width: 100%;
        z-index: 100;
        min-height: 530px;
        height: auto;
    ...
    .form-control, .form-select {
        background-color: #1a221f; /* ডার্ক ইনপুট ব্যাকগ্রাউন্ড */
    ...
    .form-control:focus, .form-select:focus {
        ...
        border-color: #d4af37; /* গোল্ড ফোকাস বর্ডার */
    ...
    #extrasSection {
        ...
        padding: 15px;
        border-radius: 12px;
        border: 1px dashed #d4af37; /* গোল্ড ড্যাশড বর্ডার */
        margin-top: 10px;
    }
    ...
    .form-header {
        background: #d4af37; /* গোল্ডেন হেডার */
        background: linear-gradient(135deg, #d4af37 0%, #aa8a2e 100%);
        color: #000; /* হেডারে টেক্সট কালো (কন্ট্রাস্টের জন্য) */
        padding: 12px;
        text-align: center;
        border-radius: 8px 8px 0 0;
    }
    .form-header h3 { margin: 0; font-size: 1.3rem; font-weight: 700; letter-spacing: 0.5px; color: #000; }
    .form-header p { margin: 0; font-size: 0.8rem; opacity: 0.8; color: #000; }
    ...
    .input-group-text { width: 40px; justify-content: center; background: #2d3833; color: #d4af37; border: 1px solid #2d3833; }
    ...
    .trip-card {
        display: flex; flex-direction: row; align-items: center; justify-content: center;
        padding: 8px; border: 1px solid #2d3833; border-radius: 6px;
        cursor: pointer; transition: all 0.2s ease; height: 100%; background: #1a221f; gap: 6px;
        font-weight: 500;
        color: #a0a0a0;
    }

    /* Trip Card Hover & Active Animation */
    .trip-card:hover { background: #242d29; border-color: #d4af37; color: #fff; transform: translateY(-2px); }
    ...
    .mini-label { font-size: 0.75rem; color: #d4af37; margin-left: 2px; font-weight: 600; display: block; margin-bottom: 4px; }
    ...
    .extras-toggle:hover { color: #f1c40f; }
    .extras-toggle i { transition: transform 0.3s ease; }
    .extras-toggle:hover i { transform: rotate(90deg); }
    ...
    .extra-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; padding-bottom: 6px; border-bottom: 1px solid #2d3833; }
    .extra-label { font-size: 0.85rem; color: #e0e0e0; font-weight: 500; }
    .extra-price-tag { background: #d4af37; color: #000; padding: 1px 6px; border-radius: 4px; font-size: 0.7rem; margin-left: 5px; font-weight: bold; }
    .total-price-display { font-weight: bold; color: #d4af37; margin-left: 10px; font-size: 0.9rem; min-width: 30px; text-align: right; }
    ...
    .btn-get-fare:hover {
        background: linear-gradient(45deg, #aa8a2e, #d4af37);
        transform: translateY(-2px);
    ...
    .footer-note { text-align: center; font-size: 0.75rem; text:bold; color: #fff; margin-top: 8px; margin-bottom: 0; }
    ...
    /* Placeholder color set to white */
        ::placeholder {
            color: #ffffff !important;
            opacity: 0.8; /* কিছুটা ট্রান্সপারেন্সি চাইলে এটি রাখতে পারেন, না চাইলে মুছে দিন */
        }
    ...
    @media (max-width: 991px) {
        .hero-section { height: auto; padding-bottom: 20px; }
        .description-column { margin-top: 30px; text-align: center; }
        .description-text { font-size: 0.95rem; margin-bottom: 20px; color: #ccc; line-height: 1.6; }
        .hero-img { height: auto; max-height: 250px; object-fit: contain; }
        .reservation-card { position: static !important; width: 100% !important; height: auto !important; }
    }
</style>
...
<script>
   ...
   function updateLuggageOption() {
    ...
    // ইউজার বর্তমানে যেটা সিলেক্ট করে রেখেছে সেটা আগে সেভ করে রাখা হচ্ছে
    const savedValue = luggageSelect.value;
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // ড্রপডাউন লিস্ট আপডেট করা হচ্ছে
            luggageSelect.innerHTML = html;

            // যদি আগের সিলেক্ট করা ভ্যালু নতুন ম্যাক্সিমাম লিমিটের ভেতরে থাকে, তবে সেটা পুনরায় সিলেক্ট করে দাও
            if (savedValue !== "" && parseInt(savedValue) <= maxLuggage) {
                luggageSelect.value = savedValue;
            } else if (savedValue !== "" && parseInt(savedValue) > maxLuggage) {
                // যদি আগের ভ্যালু নতুন লিমিটের বাইরে চলে যায় (যেমন আগে ছিল ৫, এখন ম্যাক্সিমাম ৩), তবে ৩ সিলেক্ট হবে
                luggageSelect.value = maxLuggage;
            }
        },
        error: function() {
            let html = '<option value="">Select</option>';
            for (let i = 0; i <= 12; i++) { html += `<option value="${i}">${i}</option>`; }
            luggageSelect.innerHTML = html;
            if (savedValue !== "") luggageSelect.value = savedValue;
        }
    });
}
        if (adultsSelect) adultsSelect.addEventListener('change', updateLuggageOption);
