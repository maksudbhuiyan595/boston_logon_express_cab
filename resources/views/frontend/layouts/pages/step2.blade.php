    ...
    <div class="booking-wrapper animate__animated animate__fadeIn">
        ...
        <form id="bookingForm" action="{{ route('step3') }}" method="GET">
            ...
            <input type="hidden" name="fare[extra_luggage_fee]" id="inp_elug"
                value="{{ $defaultVehicle['extra_luggage_fee'] }}">
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    <div class="vehicle-card-dark">
                        ...
                        <img src="{{ (!empty($defaultVehicle) && !empty($defaultVehicle['image'])) ? asset($defaultVehicle['image']) : asset('images/home1.jpeg') }}"
                            id="disp_img" class="v-img-main"
                            onerror="this.onerror=null;this.src='{{ asset('images/home1.jpeg') }}';">
                        ...
                        <div class="v-icons-row">
                            <div class="v-icon-box" title="Passengers">
                                ...
                                <div>{{ $reqPassengers }} Passenger</div>
                            </div>
                    ...
                    <div class="pay-box-container">
                        ...
                        <div class="pay-col">
                            ...
                            <span class="pay-amt text-red">$<span
                                    id="disp_cash">{{ number_format($defaultVehicle['pay_cash'], 2) }}</span></span>
                            <div class="pay-note">Pay $1 Reservation Fee</div>
                        </div>
                        <div class="pay-col">
                            <span class="pay-lbl">Pay Card</span>
                            <span class="pay-amt">$<span
                                    id="disp_card">{{ number_format($defaultVehicle['total_fare'], 2) }}</span></span>
                            <div class="pay-note">Full Online Payment</div>
                        </div>
                    </div>
                    <div class="bottom-note">
                        <i class="fas fa-info-circle me-1"></i> <strong>Note:</strong> Pay only $1.00 now to confirm your
                        reservation. Balance is payable after service.
                    </div>
                </div>
                    ...
                    <div class="pricing-card">
                        ...
                        <table class="pricing-table">
                            ...
                            <tr id="row_sur" style="{{ $defaultVehicle['surcharge_fee'] > 0 ? '' : 'display:none;' }}">
                                ...
                                <td>:</td>
                                <td>$<span id="tbl_sur">{{ number_format($defaultVehicle['surcharge_fee'], 2) }}</span></td>
                            </tr>

                            <tr>
                                <td class="pt-3 border-top fw-bold text-dark">Total Payable</td>
                                <td class="pt-3 border-top">:</td>
                                <td class="pt-3 border-top fw-bold text-dark fs-5">$<span
                                        id="tbl_total">{{ number_format($defaultVehicle['total_fare'], 2) }}</span></td>
                            </tr>
                        </table>

                        <div id="box_elug" class="extra-lug-box"
                            style="{{ $defaultVehicle['extra_luggage_fee'] > 0 ? '' : 'display:none;' }}">
                            <span class="fw-bold"><i class="fas fa-luggage-cart me-2"></i>Extra Luggage Fee</span>
                            <span class="fw-bold fs-5">$<span
                                    id="disp_elug">{{ number_format($defaultVehicle['extra_luggage_fee'], 2) }}</span></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="btn-pay-note text-muted">Pay only $1 & confirm your reservation. Balance is payable after service</div>

                    <div class="summary-box">
                        <div class="sum-header">Booking Details</div>
                        <table class="sum-table">
                            <tr>
                                <td>Service</td>
                                <td>:</td>
                                <td class="sum-val">
                                    {{ $trip_type == 'fromAirport' ? 'From Airport' : ($trip_type == 'toAirport' ? 'To Airport' : 'Door-to-Door') }}
                                </td>
                            </tr>
                            <tr>
                                <td>Date</td>
                                <td>:</td>
                                <td class="sum-val">{{ $formattedDate ?? $request['date'] }}</td>
                            </tr>
                            <tr>
                                <td>Time</td>
                                <td>:</td>
                                <td class="sum-val">{{ $formattedTime ?? $request['time'] }}</td>
                            </tr>
                            <tr>
                                <td>Pick up</td>
                                <td>:</td>
                                <td class="sum-val">{{ Str::limit($pickup, 28) }}</td>
                            </tr>
                            <tr>
                                <td>Drop off</td>
                                <td>:</td>
                                <td class="sum-val">{{ Str::limit($dropoff, 28) }}</td>
                            </tr>
                            <tr>
                            <td>Passengers</td>
                            <td>:</td>
                        ...
                        <tr>
                            <td>Luggage</td>
                            <td>:</td>
                            <td class="sum-val">{{ $request['luggage'] ?? 0 }}</td>
                        </tr>
                        ...
                        <tr>
                            <td>Front Facing</td>
                            <td>:</td>
                            <td class="sum-val">{{ $request['front_seat'] }}</td>
                        </tr>
                        @endif
                        ...
                        <tr>
                            <td>Stopover</td>
                            <td>:</td>
                            <td class="sum-val">{{ $request['stopover'] }}</td>
                        </tr>
                        @endif
                        @if(($request['pets'] ?? 0) > 0)
                        <tr>
                            <td>Pets</td>
                            <td>:</td>
                            <td class="sum-val">{{ $request['pets'] }}</td>
                        </tr>
                        @endif
                            ...
                            <tr>
                                <td colspan="3"
                                    style="font-weight:700; margin-bottom:10px; color:#B45309; text-transform:uppercase; font-size:0.8rem;">
                                    Vehicle Info</td>
                            </tr>
                            <tr>
                                <td>Selected</td>
                                <td>:</td>
                                <td class="sum-val"><span id="sum_vname">{{ $defaultVehicle['name'] }}</span></td>
                            </tr>
                            <tr>
                                <td>Capacity</td>
                                <td>:</td>
                                <td class="sum-val"><span id="sum_vpax">{{ $defaultVehicle['capacity_passenger'] }}</span>
                                    Pax, <span id="sum_vlug">{{ $defaultVehicle['capacity_luggage'] }}</span> Bags</td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="fw-bold fs-5" style="color:#2D9CDB;">$<span
                                        id="sum_total">{{ number_format($defaultVehicle['total_fare'], 2) }}</span></td>
                            </tr>
                        </table>
                    </div>
                </div>
                ...
                <div class="veh-row {{ $option['vehicle_id'] == $defaultVehicle['vehicle_id'] ? 'd-none-custom' : '' }}"
                    ...
                    <div class="vr-info flex-grow-1">
                        ...
                        <div class="text-muted" style="font-size:0.8rem">
                            Base Fare: ${{ number_format($option['estimated_fare'], 2) }} &bull; Gratuity:
                            ${{ number_format($option['gratuity_fee'], 2) }}
                        </div>
                    </div>
